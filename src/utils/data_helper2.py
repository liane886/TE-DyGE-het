...
def run_random_walks(G, nodes, N_WALKS=5, WALK_LEN=3):
	pairs = []
	for count, node in enumerate(nodes):
		# print(count)
		if G.degree(node) == 0:
			continue
		for i in range(N_WALKS): #对一个节点进行几次rw 
			curr_node = node
...
def load_graph(fileppath):
	...
	for i in range(len(data)):
		...
		slice_id = (current_date - START_DATE).days // (DAY_DURATION * SLICE_WEEKS)
...
def read_dynamic_graph(graph_snapshots_file, val_edge_list_file, feat_file, walk_pairs_file, normalize=True, load_walks=False):
	...
	json_str = json.dumps(id_map, indent=4)
...
def load_data(train_prefix):
	if train_prefix == "FinKG":
		edge_train_file = "utils/FinKG_id_4w.txt"
		edges_val_lr_train_test_file = "utils/val_id_4w.txt"
		walk_pairs_file = "utils/graphsage_walk_pairs_dynamic_4w_test.txt"
		# feat_file = "./feats.npz"
		# feat_file = ""
...
def load_train_test(train_prefix):
	if train_prefix == "FinKG":
		edges_val_lr_train_test_file = "utils/load_tran_test_4w.txt"
...
if __name__ == '__main__':
	# graphs = load_graph("FinKG_id1.txt")
	load_data("")
	# print(graphs)
